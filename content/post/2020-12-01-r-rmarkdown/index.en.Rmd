...
library("plotly")
```
#SET KNIT CONDITIONS FOR CHUNKS IN MARKDOWN FILE
...
write_csv(MERGED_SYM_DAT_VAX_05_21_21_151700kb, "/Users/admin/Desktop/MERGED_SYM_DAT_VAX_05_21_21_151700kb.csv")
...
```
Hospitalizations and Emergency Room Visit counts
```{r}
HOSPITAL_Y_05_21_21_151700kb  <- MERGED_SYM_DAT_VAX_05_21_21_151700kb  %>% group_by(VAERS_ID, HOSPITAL) %>% filter(HOSPITAL == "Y")
HOSPITAL_05_21_21_151700kb <- subset(HOSPITAL_Y_05_21_21_151700kb, !duplicated(subset(HOSPITAL_Y_05_21_21_151700kb, select=c(VAERS_ID))))
nrow(HOSPITAL_05_21_21_151700kb)
ER_ED_Y_05_21_21_151700kb <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% group_by(VAERS_ID, ER_ED_VISIT) %>% filter(ER_ED_VISIT == "Y")
ER_ED_05_21_21_151700kb <- subset(ER_ED_Y_05_21_21_151700kb, !duplicated(subset(ER_ED_Y_05_21_21_151700kb, select=c(VAERS_ID))))
nrow(ER_ED_05_21_21_151700kb)
```
Severe Adverse Event count and percentage and Mild Adverse Event count
```{r}
...
DEAD_N_05_21_21_151700kb <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% group_by(DIED) %>% filter(DIED != "Y")
HOSPITAL_N_05_21_21_151700kb <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% group_by(HOSPITAL) %>% filter(HOSPITAL != "Y")
ER_ED_N_05_21_21_151700kb <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% group_by(ER_ED_VISIT) %>% filter(ER_ED_VISIT != "Y")
MAES_05_21_21_151700kb <- rbind(DEAD_N_05_21_21_151700kb, HOSPITAL_N_05_21_21_151700kb, ER_ED_N_05_21_21_151700kb) %>% filter(DIED != "Y" & HOSPITAL != "Y" & ER_ED_VISIT != "Y")
MAES_no_dups_05_21_21_151700kb <- subset(MAES_05_21_21_151700kb, !duplicated(subset(MAES_05_21_21_151700kb, select=c(VAERS_ID))))
nrow(MAES_no_dups_05_21_21_151700kb)
```
...
```{r}
#strings_ear <- c("Acoustic neuritis", "Acoustic stimulation tests", "Acoustic stimulation tests abnormal", "Audiogram", "Audiogram abnormal", "Auditory disorder", "Autonomic nervous system imbalance", "Balance disorder", "Deafness", "Deafness bilateral", "Deafness Neurosensory", "Deafness transitory", "Deafness unilateral", "Dizziness", "Dizziness postural", "Ear congestion", "Ear discomfort", "Ear disorder", "Ear haemorrhage", "Ear infection", "Ear pain", "Ear pruritus", "Ear swelling", "External ear pain", "Hallucination, auditory", "Inner ear disorder", "Middle ear effusion", "Nausea", "Paraesthesia ear", "Paranasal sinus discomfort", "Paranasal sinus hypersecretion", "Sinus congestion", "Sinus disorder", "Sinus headache", "Sinus operation", "Sinus pain", "Sinusitis", "Sudden hearing loss", "Throat clearing", "Tinnitus", "Vertigo")
#strings_ear
```
```{r}
strings_gastro <- c("Abdominal adhesions", "Abdomen crushing", "Abdominal discomfort", "Abdominal distension", "Abdominal pain", "Abdominal pain lower", "Abdominal pain upper", "Abdominal rebound tenderness", "Abdominal rigidity", "Abdominal tenderness", "Abdominal wall haematoma", "Abdominal wall mass", "Abdominal wall oedema", "Abdominal X-ray", "Acute abdomen", "Bowel movement irregularity", "Choking", "Colitis", "Colonoscopy abnormal", "Colostomy", "Computerised tomogram abdomen", "Computerised tomogram abdomen abnormal", "Diarrhoea", "Diarrhoea haemorrhagic", "Diverticulum intestinal", "Diverticulitis intestinal perforated", "Dysphagia", "Enteritis", "Faeces discoloured", "Faeces pale", "Fasting", "Feeding disorder", "Flatulence", "Fluid intake reduced", "Fluid overload", "Fluid retention", "Food allergy", "Food aversion", "Food craving", "Food refusal", "Food poisoning", "Frequent bowel movements", "Gastric dilatation", "Gastric disorder", "Gastric ulcer", "Gastritis", "Gastroenteritis viral", "Gastrointestinal disorder", "Gastrointestinal haemorrhage", "Gastrointestinal inflammation", "Gastrointestinal oedema", "Gastrointestinal pain", "Gastrointestinal pathogen panel", "Gastrointestinal sounds abnormal", "Gastrointestinal wall thickening", "Gastrooesophageal reflux disease", "Gastrostomy", "Increased appetite", "Intestinal obstruction", "Intestinal resection", "Intra-abdominal haemorrhage", "Large intestine perforation", "Magnetic resonance imaging abdominal abnormal", "Mastication disorder", "Metabolic function test", "Metabolic function test abnormal", "Oesophageal discomfort", "Oesophageal disorder", "Oesophageal pain", "Oesophagogastroduodenoscopy", "Pharyngeal erythema", "Pharyngeal hypoaesthesia", "Pharyngeal oedema", "Pharyngeal paraesthesia", "Pharyngeal swelling", "Pharyngitis", "Rectal haemorrhage", "Regurgitation", "Retching", "Saliva altered", "Salivary hypersecretion", "Small intestinal obstruction", "Ultrasound abdomen", "Ultrasound abdomen abnormal", "Upper gastrointestinal haemorrhage", "Vitamin B12", "Vitamin B12 decreased", "Vitamin D decreased", "Vomiting", "Vomiting projectile", "Weight", "Weight decreased", "Weight increased")
#strings_gastro
```
```{r}
strings_liver <- c("Acute hepatic failure", "Hepatic cirrhosis", "Hepatic cyst", "Hepatic embolisation", "Hepatic encephalopathy", "Hepatic enzyme", "Hepatic enzyme decreased", "Hepatic enzyme increased", "Hepatic function abnormal", "Hepatic pain", "Hepatic steatosis", "Hepatic vein thrombosis", "Hepatitis", "Hepatitis acute", "Hepatobiliary scan", "Hepatobiliary scan abnormal", "Hepatomegaly", "Ischaemic hepatitis", "Liver abscess", "Liver disorder", "Liver function test", "Liver function test abnormal", "Liver function test increased", "Liver injury", "Metastases to liver")
```
```{r}
strings_excretion <- c("Acute kidney injury", "Albumin urine present", "Bilirubin urine", "Bilirubin urine present", "Bladder catheter replacement", "Bladder catheterisation", "Bladder hypertrophy", "Bladder injury", "Blood urine present", "Chronic kidney disease", "Creatine urine increased", "Crystal urine absent", "Crystal urine present", "Culture urine", "Culture urine positive", "Dehydration", "Escherichia urinary tract infection", "Glomerulonephritis minimal lesion", "Glucose urine present", "Haemorrhage urinary tract", "Kidney infection", "Lower urinary tract symptoms", "Micturition urgency", "Nephrolithiasis", "Nephropathy", "Nitrite urine", "Pyelonephritis", "Protein urine present", "Renal cyst", "Renal disorder", "Renal embolism", "Renal function test", "Renal impairment", "Renal pain", "Renal tubular necrosis", "Ultrasound kidney", "Urethritis", "Urinary assistance device user", "Urinary bladder haemorrhage", "Urinary casts absent", "Urinary incontinence", "Urinary occult blood positive", "Urinary retention", "Urinary sediment present", "Urinary stone analysis", "Urinary system x-ray abnormal", "Urinary tract disorder", "Urinary tract infection", "Urinary tract infection bacterial", "Urine abnormality", "Urine analysis", "Urine analysis abnormal", "Urine bilirubin decreased", "Urine electrophoresis abnormal", "Urine ketone body present", "Urine leukocyte esterase", "Urine leukocyte esterase positive", "Urine output decreased", "Urobilinogen urine", "Urobilinogen urine decreased", "Urobilinogen urine increased", "Urosepsis", "White blood cells urine positive")
#strings_excretion
```
```{r}
...
```
Cardiovascular, neurological, immunological, renal and gastrological AE counts
```{r}
...
RENAL <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% rowwise() %>% filter(SYMPTOM != "NA") %>% mutate("RENAL" = RENAL_human(SYMPTOM)) %>% filter(RENAL !="NA")
GASTRO <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% rowwise() %>% filter(SYMPTOM != "NA") %>% mutate("GASTRO" = GASTRO_human(SYMPTOM)) %>% filter(GASTRO !="NA")
...
nrow(GASTRO)
```
Breakthrough COVID-19 case count and of those, deaths and percentage deaths
```{r}
...
write_csv(MERGED_SYM_DAT_VAX_05_21_21_151700kb_covid_cases, "/Users/admin/Desktop/MERGED_SYM_DAT_VAX_05_21_21_151700kb_covid_cases.csv")
...
```
Pie chart for Breakthrough COVID-19 cases by age group
```{r}
...
```
DISTRIBUTION OF SPONTANEOUS ABORTIONS BY AGE
```{r}
...
ggsave("MERGED_SYM_DAT_VAX_05_21_21_151700kb_SA_ID_plot.png")
```
...
```
HEATMAP showing correlation between VD and OD for spontaneous abortions
```{r}
...
ggsave("SA_VD_OD_TILE_count.png")
```
...
```
```{r}
#png("MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG_plot", width = 4, height = 4, units = 'in', res = 100)

MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% rowwise() %>% filter(SYMPTOM != "NA") %>% mutate("ANAPHYLACTIC_EVENT" = anaphyl(SYMPTOM)) %>% filter(ANAPHYLACTIC_EVENT != "NA")
MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG <- MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP %>% rowwise() %>% filter(AGE_YRS != "NA") %>% mutate("AGE_GROUP" = AGE_GROUPED_BY_DECADE(AGE_YRS))
MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG_count <- MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG %>% count(AGE_GROUP)
nrow(MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP)
MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG_plot <- ggplot(MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG_count, aes(AGE_GROUP, n, fill = AGE_GROUP)) + geom_bar(stat = "Identity") + geom_text(aes(label = n)) + xlab("AGE GROUPS") + ylab("NUMBER OF ANAPHYLACTIC AEs") + theme(axis.text.y = element_text(colour = "black", size = 14)) + theme(axis.text.x = element_text(colour = "black", angle=25,vjust=0.5,hjust=0.5, size = 8)) + theme(legend.position = "none") + ggtitle("ANAPHYLACTIC EVENTS -> VAERS IDs vs AGE GROUPS") 
MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG_plot
ggsave("MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_AG_plot.png")
```
```{r}
ANAP <- data.frame(MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP, stringsAsFactors = FALSE) %>% filter(year(ONSET_DATE) <= 2021 & year(ONSET_DATE) >= 2020) 
MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_VD_OD <- ANAP %>% mutate("diff_in_days" = difftime(ONSET_DATE, VAX_DATE, units = "days")) %>% filter(diff_in_days >= 0 & diff_in_days <=25) %>% count(diff_in_days) %>% arrange(desc(n)) %>% mutate("OBSERVED" = n) %>% mutate("Percentage_OBSERVED" = n/sum(n)*100) %>% mutate("EXPECTED" = as.integer(mean(n))) %>% mutate("Percentage_EXPECTED" = EXPECTED/nrow(ANAP)*100)
head(MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_VD_OD)

chisq.test(MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_VD_OD$OBSERVED)
```
#ANAP HEATMAP
```{r}
ANAP_VD_OD_TILE_count <- MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP %>% group_by(ONSET_DATE, VAX_DATE) %>% filter(year(ONSET_DATE) <= 2021 & year(ONSET_DATE) >= 2021) %>% filter(year(VAX_DATE) <= 2021 & year(VAX_DATE) >= 2021)%>% mutate("diff_in_days" = difftime(ONSET_DATE, VAX_DATE, units = "days")) %>% filter(diff_in_days >= 0 & diff_in_days <=50) %>% count(diff_in_days) 

#png("ANAP_VD_OD_TILE_count_plot ", width = 4, height = 4, units = 'in', res = 100)

ANAP_VD_OD_TILE_count_plot <- ggplot(ANAP_VD_OD_TILE_count, aes(VAX_DATE, ONSET_DATE)) + geom_tile(aes(fill = n)) + scale_fill_gradient('n', low = "blue", high = "red") + theme(panel.background = element_rect(fill = "black", color = 'black')) + ylab("ONSET OF SPONTANEOUS ABORTION") + xlab("VACCINE DATE") + theme(panel.grid=element_blank()) + theme(axis.text.x = element_text(colour = "black", size = 14)) + theme(axis.text.y = element_text(colour = "black", size = 14), axis.text.x=element_text(angle=0,vjust=0.5,hjust=0.5)) + ggtitle("VACCINE DATE vs ANAPHYLAXIS DATE") 
ANAP_VD_OD_TILE_count_plot 
ggsave("ANAP_VD_OD_TILE_count_plot.png")
```
```{r}
#png("CAUSATION_ANAP_plot", width = 4, height = 4, units = 'in', res = 100)

CAUSATION_ANAP_plot <- ggplot() + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_VD_OD, aes(diff_in_days, Percentage_OBSERVED), colour = "orange", size = 2) + ylab("PERCENTAGE REPORTED ANAPHYLACTIC AEs") + xlab("NUMBER OF DAYS FOLLOWING INJECTION") + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_ANAP_VD_OD, aes(diff_in_days, Percentage_EXPECTED), colour = "yellow", size = 2) + labs(title = "ANAPHYLAXIS EVENTS post injection", caption = "Data source: VAERS") + theme(axis.text.y = element_text(colour = "black", size = 12)) + theme(axis.text.x = element_text(colour = "black", angle=0,vjust=0.5,hjust=0.5, size = 12)) + theme(panel.background = element_rect(fill = "#100000")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) 
CAUSATION_ANAP_plot 
...
```
Percentage_OBSERVED AGAINST Percentage_EXPECTED plots to show causation in grouped AEs
```{r}
...
```
```{r}
H <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% filter(HOSPITAL == "Y") %>% filter(year(ONSET_DATE) <= 2021 & year(ONSET_DATE) >= 2020)
MERGED_SYM_DAT_VAX_05_21_21_151700kb_H_VD_OD <- H %>% mutate("diff_in_days" = difftime(ONSET_DATE, VAX_DATE, units = "days")) %>% filter(diff_in_days >= 0 & diff_in_days <=50) %>% count(diff_in_days) %>% arrange(desc(n))  %>% mutate("OBSERVED" = n)  %>% mutate("Percentage_OBSERVED" = n/sum(n)*100) %>% mutate("EXPECTED" = mean(n)) %>% mutate("Percentage_EXPECTED" = EXPECTED/nrow(H)*100)
head(MERGED_SYM_DAT_VAX_05_21_21_151700kb_H_VD_OD)

chisq.test(MERGED_SYM_DAT_VAX_05_21_21_151700kb_H_VD_OD$OBSERVED)
```
#NOW I WOULD LIKE TO PLOT Percentage_OBSERVED AGAINST Percentage_EXPECTED
```{r}
#png("CAUSATION_HOSP_plot", width = 4, height = 4, units = 'in', res = 100)

CAUSATION_HOSP_plot <- ggplot() + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_H_VD_OD, aes(diff_in_days, Percentage_OBSERVED), colour = "orange", size = 2) + xlab("NUMBER OF DAYS FOLLOWING INJECTION") +  ylab("PERCENTAGE OF REPORTED HOSPITALIZATIONS") + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_H_VD_OD, aes(diff_in_days, Percentage_EXPECTED), colour = "yellow", size = 2) + labs(title = "HOSPITALIZATIONS post injection", caption = "Data source: VAERS") + theme(axis.text.y = element_text(colour = "black", size = 14)) + theme(axis.text.x = element_text(colour = "black", angle=0,vjust=0.5,hjust=0.5, size = 14)) + theme(panel.background = element_rect(fill = "#100000", color = "pink")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
CAUSATION_HOSP_plot
ggsave("CAUSATION_HOSP_plot.png")
```
#HOW ABOUT HEATMAPS?
```{r}
H_VD_OD_TILE_count <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% group_by(HOSPITAL, ONSET_DATE, VAX_DATE) %>% filter(HOSPITAL == "Y") %>% filter(year(ONSET_DATE) <= 2021 & year(ONSET_DATE) >= 2021) %>% filter(year(VAX_DATE) <= 2021 & year(VAX_DATE) >= 2021)%>% mutate("diff_in_days" = difftime(ONSET_DATE, VAX_DATE, units = "days")) %>% filter(diff_in_days >= 0 & diff_in_days <=50) %>% count(diff_in_days) 

#png("H_VD_DD_TILE_plot ", width = 4, height = 4, units = 'in', res = 100)
...
```
```{r}
ER <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% filter(ER_ED_VISIT == "Y") %>% filter(year(ONSET_DATE) <= 2021 & year(ONSET_DATE) >= 2020)
MERGED_SYM_DAT_VAX_05_21_21_151700kb_ER_VD_OD <- ER %>% mutate("diff_in_days" = difftime(ONSET_DATE, VAX_DATE, units = "days")) %>% filter(diff_in_days >= 0 & diff_in_days <=50) %>% count(diff_in_days) %>% arrange(desc(n))  %>% mutate("OBSERVED" = n) %>% mutate("Percentage_OBSERVED" = n/sum(n)*100) %>% mutate("EXPECTED" = mean(n)) %>% mutate("Percentage_EXPECTED" = EXPECTED/nrow(ER)*100)
head(MERGED_SYM_DAT_VAX_05_21_21_151700kb_ER_VD_OD)

chisq.test(MERGED_SYM_DAT_VAX_05_21_21_151700kb_ER_VD_OD$OBSERVED)
```
#NOW I WOULD LIKE TO PLOT Percentage_OBSERVED AGAINST Percentage_EXPECTED
```{r}
#png("CAUSATION_ER_plot", width = 4, height = 4, units = 'in', res = 100)

CAUSATION_ER_plot <- ggplot() + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_ER_VD_OD, aes(diff_in_days, Percentage_OBSERVED), colour = "orange", size = 2) + xlab("NUMBER OF DAYS FOLLOWING INJECTION") +  ylab("PERCENTAGE OF REPORTED ER VISITS") + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_ER_VD_OD, aes(diff_in_days, Percentage_EXPECTED), colour = "yellow", size = 2) + labs(title = "ER VISITS post injection", caption = "Data source: VAERS") + theme(axis.text.y = element_text(colour = "black", size = 14)) + theme(axis.text.x = element_text(colour = "black", angle=0,vjust=0.5,hjust=0.5, size = 14)) + theme(panel.background = element_rect(fill = "#100000", color = "pink")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
CAUSATION_ER_plot 
ggsave("CAUSATION_ER_plot.png")
```
#HOW ABOUT HEATMAPS?
```{r}
ER_VD_OD_TILE_count <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% group_by(ER_ED_VISIT, ONSET_DATE, VAX_DATE) %>% filter(ER_ED_VISIT == "Y") %>% filter(year(ONSET_DATE) <= 2021 & year(ONSET_DATE) >= 2021) %>% filter(year(VAX_DATE) <= 2021 & year(VAX_DATE) >= 2021)%>% mutate("diff_in_days" = difftime(ONSET_DATE, VAX_DATE, units = "days")) %>% filter(diff_in_days >= 0 & diff_in_days <=50) %>% count(diff_in_days) 

#png("ER_VD_DD_TILE_plot ", width = 4, height = 4, units = 'in', res = 100)
...
CAUSATION_NEURO_plot <- ggplot() + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_NEURO_VD_OD, aes(diff_in_days, Percentage_OBSERVED), colour = "orange", size = 2) + xlab("NUMBER OF DAYS FOLLOWING INJECTION") +  ylab("PERCENTAGE OF REPORTED NEURO AEs") + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_ER_VD_OD, aes(diff_in_days, Percentage_EXPECTED), colour = "yellow", size = 2) + labs(title = "NEUROLOGICAL AEs post injection", caption = "Data source: VAERS") + theme(axis.text.y = element_text(colour = "black", size = 14)) + theme(axis.text.x = element_text(colour = "black", angle=0,vjust=0.5,hjust=0.5, size = 14)) + theme(panel.background = element_rect(fill = "#100000", color = "pink")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + ylim(0,50)
...
CAUSATION_IMMUNO_plot <- ggplot() + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_IMMUNO_VD_OD, aes(diff_in_days, Percentage_OBSERVED), colour = "orange", size = 2) + xlab("NUMBER OF DAYS FOLLOWING INJECTION") +  ylab("PERCENTAGE OF REPORTED IMMUNO AEs") + geom_line(data = MERGED_SYM_DAT_VAX_05_21_21_151700kb_ER_VD_OD, aes(diff_in_days, Percentage_EXPECTED), colour = "yellow", size = 2) + labs(title = "IMMUNOLOGICAL AEs post injection", caption = "Data source: VAERS") + theme(axis.text.y = element_text(colour = "black", size = 14)) + theme(axis.text.x = element_text(colour = "black", angle=0,vjust=0.5,hjust=0.5, size = 14)) + theme(panel.background = element_rect(fill = "#100000", color = "pink")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
```
```{r}
#png("AGE_GROUPS_H_ALL_ID_plot", width = 4, height = 4, units = 'in', res = 100)
...
```
```{r}
#png("AGE_GROUPS_ER_ALL_ID_plot", width = 4, height = 4, units = 'in', res = 100)
...
ggsave("MERGED_SYM_DAT_VAX_05_21_21_151700kb_count_plot.png")
...
```
```{r}
#MERGED_SYM_DAT_VAX_05_21_21_151700kb$VAX_DOSE_SERIES <- as.integer(MERGED_SYM_DAT_VAX_05_21_21_151700kb$VAX_DOSE_SERIES) 
MERGED_SYM_DAT_VAX_05_21_21_151700kb_doses <- MERGED_SYM_DAT_VAX_05_21_21_151700kb %>% group_by(VAX_DOSE_SERIES, VAX_MANU) %>% filter(VAX_DOSE_SERIES >= 0 & VAX_DOSE_SERIES <= 2 & VAX_MANU != "UNKNOWN MANUFACTURER") %>% count(VAX_DOSE_SERIES) %>% arrange(desc(n)) 
MERGED_SYM_DAT_VAX_05_21_21_151700kb_doses_n <- MERGED_SYM_DAT_VAX_05_21_21_151700kb_doses %>% mutate("SUM" = sum(MERGED_SYM_DAT_VAX_05_21_21_151700kb_doses$n)) %>% mutate("PERCENT" = n/SUM*100)
MERGED_SYM_DAT_VAX_05_21_21_151700kb_doses_n
```
...
#head(datatable(Table_MOH_Israel))
```
```{r}
Table_MOH_Israel <- Table_MOH_Israel %>% mutate("FV_per_million" = FV/1000)
#Table_MOH_Israel
```
TIME SERIES PLOT FOR MOH DATA 
from Fully Vaccinated population and from deaths due to COVID-19
```{r}
#png("Table_MOH_Israel.png", width = 4, height = 4, units = 'in', res = 100)

Table_MOH_Israel_plot_D <- ggplot(data = Table_MOH_Israel, aes(x = DATE, y = DEATH)) + geom_line(data = Table_MOH_Israel, aes(x = DATE, y = DEATH, colour = "DEATH"), colour = "red", size = 2) + geom_point(data = Table_MOH_Israel, aes(DATE, DEATH, colour = "DEATH"), colour = "red", size = 5) + geom_point(data = Table_MOH_Israel, aes(DATE, FV_per_million, colour = "FV_per_million"), colour = "yellow", size = 5) + geom_line(data = Table_MOH_Israel, aes(DATE, FV_per_million, colour = "FV_per_million"), colour = "yellow", size = 2)+ xlab("DATE") +  ylab("# DEAD (red) and FULLY VAXXED (yellow)") + labs(title = "Deaths versus fully vaxxed population in Israel", caption = "Data source: ISRAEL MINISTRY OF HEALTH") + theme(axis.text.y = element_text(colour = "black", size = 14)) + theme(axis.text.x = element_text(colour = "black", angle=15,vjust=0.5,hjust=0.5, size = 12)) + theme(panel.background = element_rect(fill = "black")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + theme(legend.title=element_blank()) + scale_x_continuous(breaks = round(seq(min(Table_MOH_Israel$DATE), max(Table_MOH_Israel$DATE), by = 10),0.1))
Table_MOH_Israel_plot_D
ggsave("Table_MOH_Israel_plot_D.png")
```
```{r}
#png("Table_MOH_Israel.png", width = 4, height = 4, units = 'in', res = 100)
...
Table_MOH_Israel_plot_FV <- ggplot(data = Table_MOH_Israel, aes(x = DATE, y = DEATH)) + geom_line(data = Table_MOH_Israel, aes(x = DATE, y = FV, colour = "FV"), colour = "red", size = 2) + geom_point(data = Table_MOH_Israel, aes(DATE, FV, colour = "FV"), colour = "red", size = 5) + xlab("DATE") +  ylab("NUMBER OF FULLY-VACCINATED") + labs(caption = "Data source: ISRAEL MINISTRY OF HEALTH") + theme(axis.text.y = element_text(colour = "black", size = 14)) + theme(axis.text.x = element_text(colour = "black", angle=15,vjust=0.5,hjust=0.5, size = 12)) + theme(panel.background = element_rect(fill = "black")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + theme(legend.title=element_blank()) + scale_x_continuous(breaks = round(seq(min(Table_MOH_Israel$DATE), max(Table_MOH_Israel$DATE), by = 10),0.1)) + ylim(0,6000000)
Table_MOH_Israel_plot_FV
ggsave("Table_MOH_Israel_plot_FV.png")
```