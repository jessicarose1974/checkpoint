...
```
```{r}
FRI_CASES_count_plot <- ggplot(FRI_CASES_perc, aes(n, SYMPTOM, fill = SYMPTOM)) + geom_bar(stat = "Identity") + geom_text(aes(label = n)) + ylab("SYMPTOM") + xlab("Number of Female Reproductive Disorders") + theme(axis.text.y = element_text(colour = "black", size = 6)) + theme(axis.text.x = element_text(colour = "black", angle=0,vjust=0.5,hjust=0.5, size = 10)) + theme(legend.position = "none") + ggtitle("Female Reproductive Adverse Events")
FRI_CASES_count_plot
```